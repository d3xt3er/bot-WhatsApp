...
(async function () {
  ...
  const familiares = grupos.filter(grupo => grupo.formattedTitle.indexOf("Bando") !== -1);

  for(let index = 0; index < familiares.length; index++){
      await cliente.sendFile(familiares[index].id._serialized, imagemBase64, 'bomdia.jpeg', "ENVIADO DO ROBÔ DO PAULO");
  }
